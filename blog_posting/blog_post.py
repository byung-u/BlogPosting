...
class BlogPost:
    ...
    def request_and_get(self, url):
        try:
            r = get(url, headers={'User-Agent': choice(USER_AGENTS)})
            if r.status_code != codes.ok:
                self.logger.error('[%s] request error, code=%d', url, r.status_code)
                return False
            return r
        except TypeError:
            self.logger.error('[%s] connect fail', url)
            return False

    def get_news_article_info(self, url):
        article = Article(url)
        article.download()
        try:
            article.parse()
